...
import { ArrowRight, Check, ChevronDown, CircleHelp, Clock3, Copy, Edit3, ExternalLink, Gamepad2, LayoutDashboard, MonitorPlay, Play, Plus, RotateCcw, ShieldCheck, Sparkles, Trophy, Users } from 'lucide-react'
import { anagramDisplay, currentQuestion, gradeFor, isAnswerComplete, ranked, rankedRound, responseFor, roundPoints, scrambleWord, typeInstructions, typeNames, type Game, type Player, type Question } from './model'
...
import { beginHostRedirect, finishHostRedirect, followLiveScreen, hasHostSession, hostRedirectKey, joinLiveGame, liveHostCommand, reconnectLivePlayer, startLiveHost, submitLiveAnswer, takeLiveControl } from './live'
import { answerLabel, ownResultForGame, type OwnResult } from './reveal'
...
function PlayerPortalQr({ value }: { value: string }) {
  ...
  return src ? <img className="player-portal-qr" src={src} alt="QR code for the QuizForge Player Portal"/> : null
}
...
function Home() {
  ...
  return <div className="home"><div className="home-nav"><Logo/><div><Link to="/organiser">Explore workspace</Link><Link to="/join" className="nav-join">Join a game <ArrowRight size={15}/></Link></div></div><main className="home-main"><div className="home-copy"><div className="eyebrow"><span/> A NEW WAY TO PLAY TOGETHER</div><h1>Quiz night,<br/><em>made brilliant.</em></h1><p>Create a quiz, bring everyone in with a code, and run the whole room from one calm host console.</p><div className="home-buttons"><Button onClick={() => nav('/host')}><Play size={18}/> Host a Game</Button><Button variant="secondary" onClick={() => nav('/join')}><Users size={18}/> Join a Game</Button></div><div className="home-proof"><div><strong>12</strong><span>question formats</span></div><div><strong>12</strong><span>avatar packs</span></div><div><strong>50</strong><span>players planned</span></div></div></div><div className="home-art"><div className="art-orbit orbit-one"/><div className="art-orbit orbit-two"/><div className="art-glow"/><img className="art-platypus" src={asset('avatars/platypus/wizard.png')} alt="Wizard platypus pixel art"/><img className="art-orb" src={asset('avatars/default/purple.png')} alt="Purple avatar"/><div className="floating-card"><Trophy size={18}/> <span>Round 1 is live<strong>Players are answering</strong></span></div></div></main><div className="home-foot">Build. Host. Play. <span>✦</span> QuizForge</div></div>
}
function Organiser() {
...
function Host() {
  ...
  const [liveStatus, setLiveStatus] = useState(''), [canControl, setCanControl] = useState(false), [liveBusy, setLiveBusy] = useState(false), [popupClosed, setPopupClosed] = useState(false)
  ...
  const copy = async () => { await navigator.clipboard.writeText(`${location.origin}${location.pathname}#/join`);setCopied(true);setTimeout(()=>setCopied(false),1800) }
  const startLive = async () => {
    setLiveBusy(true)
    setLiveStatus('Opening Google sign-in…')
    try {
      stopLive.current?.()
      stopLive.current = await startLiveHost((message, control) => { setLiveStatus(message); setCanControl(control) })
      setPopupClosed(false)
    }
    catch (error) {
      const closed = (error as { code?: string }).code === 'auth/popup-closed-by-user'
      setPopupClosed(closed)
      setLiveStatus(closed ? 'The browser closed the Google sign-in window before QuizForge received your account. Try full-page sign-in.' : `Live setup failed: ${(error as Error).message}`)
    }
  ...
  useEffect(() => {
    ...
    const restore = async () => {
      try {
        ...
        if (cancelled) stop()
        else { stopLive.current = stop; setPopupClosed(false) }
      } catch (error) {
    ...
    void restore()
    return () => { cancelled = true; stopLive.current?.(); stopLive.current = null }
  }, [])
  const redirectHost = async () => {
    setLiveBusy(true)
    setLiveStatus('Opening full-page Google sign-in…')
    try { await beginHostRedirect() }
    catch (error) { setLiveStatus(`Full-page sign-in failed: ${(error as Error).message}`); setLiveBusy(false) }
  }
  ...
  return <Shell active="Host console"><main className="page host-page"><div className="page-heading"><div><div className="eyebrow dark">LIVE HOST CONSOLE</div><h1>{game.title}</h1><p>{getLiveRole()==='host'?'Live across devices. Share the Player Portal and tell players the current code.':'Local demo only. Enable device sync before sharing the Player Portal with another device.'}</p></div><div className="host-head-actions"><Link className="btn secondary" to={getLiveRole()==="host"?`/screen/${game.code}`:"/screen"} target="_blank"><MonitorPlay size={17}/> Open main screen</Link><Button variant="secondary" onClick={()=>void startLive()} disabled={liveBusy || getLiveRole()==="host"}>{getLiveRole()==="host"?"Live connected":"Enable device sync"}</Button>{getLiveRole()==='host'&&<Button variant="secondary" onClick={()=>void startFreshLive()} disabled={liveBusy}>New live game</Button>}<Button variant="secondary" onClick={copy}><Copy size={16}/>{copied?'Portal copied':'Copy player portal'}</Button></div></div>{liveStatus&&<div className="hint" role="status">{liveStatus}{popupClosed&&<button onClick={redirectHost} disabled={liveBusy}>Try full-page sign-in</button>}{getLiveRole()==="host"&&!canControl&&<button onClick={async()=>{try{await takeLiveControl()}catch(error){setLiveStatus((error as Error).message)}}}>Take Control</button>}</div>}<div className="host-status"><div><span className="status-orb"><Play size={18}/></span><div><small>CURRENT SCREEN</small><strong>{phaseNames[game.phase]}</strong></div></div><span className="host-code">GAME CODE <b>{game.code}</b></span><Countdown game={game} className="host-timer"/><span className="host-count"><Users size={18}/>{game.players.length} players</span></div><div className="host-grid"><div className="host-main"><div className="host-question"><div className="host-q-top"><Badge>{q?.round || 'ROUND 1'}</Badge><span>QUESTION {game.questionIndex+1} / {game.questions.length}</span></div><h2>{q?.prompt}</h2><div className="host-q-meta"><span><Clock3 size={16}/>{q?.duration || 30}s timer</span><span><Trophy size={16}/>{q?.points} pts</span><span>{q ? typeNames[q.type] : ''}</span></div>{q && <HostAnswerKey question={q}/>}{q?.options && <div className="host-options">{q.options.map((o,i)=><div key={o}><span>{'ABCD'[i]}</span>{o}</div>)}</div>}</div><div className="control-card"><div><h3>Game controls</h3><p>Each action changes the presentation and player screens.</p></div><div className="control-buttons"><Button onClick={()=>getLiveRole()==='host'?liveAction(game.phase==='break'?'resume':'advance'):game.phase==='break'?resumeBreak(game.stateVersion):hostAction(game.stateVersion)} disabled={game.phase==='closed-game'||liveBusy||(getLiveRole()==='host'&&!canControl)}>{next}<ArrowRight size={18}/></Button>{['scores','round-scores','leaderboard','round-intro'].includes(game.phase)&&<Button variant="secondary" onClick={()=>getLiveRole()==="host"?liveAction("break"):takeBreak(game.stateVersion)}>Take a break</Button>}{['question','open','closed','reveal'].includes(game.phase)&&<Button variant="danger" onClick={()=>{if (confirm('Void this question? Its points will be removed.')) { if (getLiveRole()==='host') void liveAction('void'); else voidQuestion() }}}>Void question</Button>}</div></div><div className="response-card"><div className="card-title"><h3>Incoming answers</h3><Badge tone={game.phase==='open'?'green':'gray'}>{answered.length} / {game.players.length} submitted</Badge></div>{answered.length===0?<div className="empty-state">Player answers will appear here while the question is open.</div>:<div className="answer-list">{answered.map(r=>{const p=game.players.find(p=>p.id===r.playerId);if(!p)return null;const g=gradeFor(game,p.id,q.id);return <div key={p.id} className="answer-row"><div className="answer-player"><PlayerAvatar player={p} packs={packs}/><span><strong>{p.name}</strong><small>{new Date(r.submittedAt).toLocaleTimeString()}</small></span></div><span className="response-value">{typeof r.value==='object'?JSON.stringify(r.value):String(r.value)}</span>{['free','text'].includes(q.type)&&<div className="mark-buttons"><button onClick={()=>getLiveRole()==="host"?liveAction("grade",{playerId:p.id,points:q.points}):setGrade(p.id,q.points)}>✓</button><button onClick={()=>getLiveRole()==="host"?liveAction("grade",{playerId:p.id,points:0}):setGrade(p.id,0)}>✕</button></div>}{g&&<Badge tone={g.points?'green':'gray'}>{g.points} pts</Badge>}</div>})}</div>}</div></div><div className="host-side"><div className="side-card"><div className="card-title"><h3>Overall leaderboard</h3><Trophy size={19}/></div>{game.players.length===0?<p className="muted">Waiting for the first player to join.</p>:ranked(game.players).map((p,i)=><div className="rank-row" key={p.id}><span className="rank-num">{i+1}</span><PlayerAvatar player={p} packs={packs} size={34}/><strong>{p.name}</strong><b>{p.score}</b></div>)}</div><div className="side-card helper"><h3>Try the full flow</h3><ol><li>Open the main screen.</li><li>Open the Player Portal and enter the code.</li><li>Start the quiz and open answers.</li><li>Submit, reveal and score.</li></ol><Link to="/join" target="_blank">Open Player Portal <ExternalLink size={15}/></Link></div><div className="side-card demo-jump"><h3>Jump to a question</h3><p className="muted">Demo shortcut to try any question type.</p><select aria-label="Jump to question" value={game.questionIndex} disabled={getLiveRole()==="host"} onChange={e=>jumpToQuestion(Number(e.target.value))}>{game.questions.map((item,index)=><option key={item.id} value={index}>{index+1}. {typeNames[item.type]}</option>)}</select></div><Button variant="ghost" disabled={getLiveRole()==="host"} onClick={()=>{if(confirm('Reset this demo game and remove demo players?'))resetGame()}}><RotateCcw size={16}/> Reset demo</Button></div></div></main></Shell>
}
function MainScreen() {
  const { code } = useParams()
  const [connectionError, setConnectionError] = useState('')
  const [connectedCode, setConnectedCode] = useState('')
  useEffect(() => code ? followLiveScreen(code, setConnectionError, () => setConnectedCode(code)) : undefined, [code])
...
function Join() {
  ...
  const displayedResult = player && (game.phase === 'reveal' || ['scores','round-scores','leaderboard','final'].includes(game.phase)) ? (getLiveRole()==='player' ? ownResult : ownResultForGame(game, player.id)) : null
  ...
  const join=async()=>{
    ...
    try {
      ...
      const liveId = tryLive ? await joinLiveGame(code,name,selected,setReadyQuestionId,setOwnResult,setError) : null
      if (liveId) { setPlayerId(liveId); window.history.replaceState(null, '', `${location.pathname}#/join/${code.trim().toUpperCase()}`) }
