...
import { anagramDisplay, currentQuestion, gradeFor, isAnswerComplete, ranked, rankedRound, responseFor, roundPoints, scrambleWord, typeInstructions, typeNames, type Game, type Player, type Question, type QuestionType } from './model'
import { actionLabel, createQuiz, deleteQuiz, duplicateQuiz, expireAnswers, getActiveQuizTemplate, getLiveRole, importQuiz, leaveLiveRole, replaceQuizLibrary, selectQuiz, update, useGame, useQuizLibrary, type QuizTemplate } from './store'
...
function readQuizPack(text: string): { title: string; questions: Question[] } {
...
function Home() {
  ...
  return <div className="home"><div className="home-nav"><Logo/><div><Link to="/organiser">Explore workspace</Link><Link to="/join" className="nav-join">Join a game <ArrowRight size={15}/></Link></div></div><main className="home-main"><div className="home-copy"><div className="eyebrow"><span/> A NEW WAY TO PLAY TOGETHER</div><h1>Quiz night,<br/><em>made brilliant.</em></h1><p>Create a quiz, bring everyone in with a code, and run the whole room from one calm host console.</p><div className="home-buttons"><Button onClick={() => nav('/host')}><Play size={18}/> Host a Game</Button><Button variant="secondary" onClick={() => nav('/join')}><Users size={18}/> Join a Game</Button></div><div className="home-proof"><div><strong>{questionTypes.length}</strong><span>question formats</span></div><div><strong>12</strong><span>avatar packs</span></div><div><strong>50</strong><span>players planned</span></div></div></div><div className="home-art"><div className="art-orbit orbit-one"/><div className="art-orbit orbit-two"/><div className="art-glow"/><img className="art-platypus" src={asset('avatars/platypus/wizard.png')} alt="Wizard platypus pixel art"/><img className="art-orb" src={asset('avatars/default/purple.png')} alt="Purple avatar"/><div className="floating-card"><Trophy size={18}/> <span>Round 1 is live<strong>Players are answering</strong></span></div></div></main><div className="home-foot">Build. Host. Play. <span>✦</span> QuizForge</div></div>
}
function Organiser() {
  ...
  const exportQuiz = (quiz: QuizTemplate) => {
    const payload = JSON.stringify({ format: 'quizforge-pack', version: 1, exportedAt: new Date().toISOString(), quiz: { title: quiz.title, questions: quiz.questions } }, null, 2)
  ...
  const importQuizPack = async (file?: File) => {
    ...
    try {
      ...
      const quiz = importQuiz(pack.title, pack.questions)
      const signedIn = await hasAdminSession()
      if (signedIn) {
  ...
  return <Shell active="Overview"><main className="page">
    ...
    <div className="quiz-library">{library.quizzes.map(quiz => {
      ...
      return <div className={`quiz-card ${selected ? 'selected-quiz' : ''}`} key={quiz.id}><div className="quiz-cover"><div className="cover-orbit"/><Sparkles size={38}/></div><div className="quiz-details"><div className="quiz-badges"><Badge tone={selected?'green':'gray'}>{selected?'SELECTED':'READY'}</Badge>{quiz.builtIn&&<Badge tone="amber">TEST QUIZ</Badge>}</div><h3>{quiz.title}</h3><p>{rounds} {rounds === 1 ? 'round' : 'rounds'} · {quiz.questions.length} {quiz.questions.length === 1 ? 'question' : 'questions'}</p><div className="quiz-card-actions"><Button onClick={() => openQuiz(quiz.id,'/host')}><Play size={17}/> Host this quiz</Button>{!quiz.builtIn&&<Button variant="secondary" onClick={() => openQuiz(quiz.id,'/editor')}><Edit3 size={16}/> Edit quiz</Button>}<Button variant="ghost" onClick={()=>exportQuiz(quiz)}><Download size={15}/> Export</Button><Button variant="ghost" onClick={()=>void copyQuiz(quiz.id)}><Copy size={15}/>{quiz.builtIn?'Copy to edit':'Duplicate'}</Button>{!quiz.builtIn&&<Button variant="danger" onClick={()=>void removeQuiz(quiz.id)}><Trash2 size={15}/> Delete</Button>}</div></div></div>
    })}</div>
    <div className="section-title lower"><h2>Workspace tools</h2></div><div className="feature-grid"><Link to="/join" className="feature-card"><div className="feature-icon lilac"><Users size={21}/></div><h3>Player Portal</h3><p>The permanent page players bookmark, scan and use to enter each game code.</p><span>Open Player Portal <ArrowRight size={16}/></span></Link><Link to="/screen" className="feature-card"><div className="feature-icon coral"><MonitorPlay size={21}/></div><h3>Screen launcher</h3><p>Enter a live session code to load its presentation on any display.</p><span>Open screen launcher <ArrowRight size={16}/></span></Link><div className="feature-card"><div className="feature-icon mint"><Sparkles size={21}/></div><h3>Avatar collection</h3><p>{packs.reduce((total,pack) => total+pack.avatars.length,0)} characters across {packs.length} avatar packs.</p><span>Available to every player <Check size={16}/></span></div></div>
  </main></Shell>
}
function Editor() {
  const game = useGame(), library = useQuizLibrary(), nav = useNavigate(), [selected, setSelected] = useState(0), [draft, setDraft] = useState(game.questions[0]), [titleDraft, setTitleDraft] = useState(game.title), [saved, setSaved] = useState(false), [validation, setValidation] = useState(''), [imageBusy, setImageBusy] = useState(false)
  useEffect(() => { setDraft(game.questions[selected] || game.questions[0]); setSaved(false) }, [selected, game.questions])
  useEffect(() => { setTitleDraft(game.title) }, [game.title])
  ...
  const save = async () => {
    ...
    update(gameDraft => { gameDraft.title = titleDraft.trim(); gameDraft.questions[selected] = prepared })
  ...
  const addQuestion = () => {
    ...
    const question: Question = { id: crypto.randomUUID(), round: draft.round || 'ROUND 1', type: 'single', prompt: 'New question', options: ['Answer A', 'Answer B', 'Answer C', 'Answer D'], answer: 'Answer A', points: 1000, duration: 30 }
  ...
  return <Shell active="Quiz editor"><main className="page editor-page">
    ...
    <div className="editor-grid">
      ...
      <div className="editor-form">
        ...
        <label>Quiz name<input value={titleDraft} onChange={event=>setTitleDraft(event.target.value)} placeholder="My brilliant quiz"/></label>
  ...
  return <Shell active="Host console"><main className="page host-page">
    <div className="page-heading">
      ...
      <div className="host-head-actions">
        {!liveConnected ? <Button onClick={()=>void startLive()} disabled={liveBusy}><Play size={17}/>{liveBusy ? 'Connecting…' : 'Start live session'}</Button> : <>
          ...
          <Button variant="secondary" onClick={()=>void copy('portal')}><Copy size={16}/>{copied==='portal' ? 'Portal copied' : 'Copy Player Portal'}</Button>
    ...
    <div className="host-grid">
      ...
      <div className="host-side">
        ...
        <div className="side-card helper"><h3>Live session order</h3><ol><li>Start the live session.</li><li>Open the supplied Main Screen link.</li><li>Players enter the supplied code.</li><li>Press Start quiz when the room is ready.</li></ol><Link to="/join" target="_blank">Open Player Portal <ExternalLink size={15}/></Link></div>
...
function MainScreen() {
  ...
  if (!code) return <div className="screen screen-launcher"><div className="screen-launcher-card"><Logo/><div className="eyebrow">PRESENTATION DISPLAY</div><h1>Connect this screen</h1><p>Enter the code created by the Host console. This display will then follow that live game automatically.</p><label>LIVE GAME CODE<input autoFocus maxLength={6} value={enteredCode} onChange={event=>setEnteredCode(event.target.value.toUpperCase())} onKeyDown={event=>event.key==='Enter'&&openCode()} placeholder="ABC123"/></label><Button onClick={openCode} disabled={!enteredCode.trim()}>Load Main Screen <ArrowRight size={18}/></Button><small>The game-specific link from the Host console fills this in automatically.</small></div></div>
