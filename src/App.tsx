...
import { anagramDisplay, currentQuestion, currentTheme, gradeFor, isAnswerComplete, normalise, quizThemes, ranked, rankedRound, responseFor, roundPoints, scrambleWord, typeNames, type Game, type Player, type Question, type QuestionType, type QuizTheme } from './model'
...
import { normaliseQuestion, questionInstruction, scoringSummary } from './scoring'
import './App.css'
import './brand.css'
...
function changeQuestionType(question: Question, type: QuestionType): Question {
  ...
  if (type === 'boolean') return { ...next, answer: true, points: 750 }
  if (type === 'number' || type === 'closest') return { ...next, answer: 0, numberBands: type === 'number' ? [{ tolerance: 0, fraction: 1 }] : undefined }
...
function validateQuestionDraft(question: Question) {
  ...
  if (question.type === 'number') {
    ...
    if (!bands.length) return 'Add at least one number scoring band.'
    if (bands.some(band => !Number.isFinite(band.tolerance) || band.tolerance < 0 || !Number.isFinite(band.fraction) || band.fraction <= 0 || band.fraction > 1)) return 'Number bands need a non-negative tolerance and score fraction from 0.01 to 1.'
    if (new Set(bands.map(band => band.tolerance)).size !== bands.length) return 'Each number scoring tolerance must be unique.'
    const sortedBands = [...bands].sort((one, two) => one.tolerance - two.tolerance)
    if (sortedBands.some((band, index) => index > 0 && band.fraction > sortedBands[index - 1].fraction)) return 'Wider number tolerances cannot award more than narrower tolerances.'
  }
...
function PlayerReveal({ question, response, result }: { question: Question; response: Game['responses'][number] | undefined; result: OwnResult | null }) {
  const title = !result ? 'Checking your result…' : result.verdict === 'correct' ? 'You got it!' : result.verdict === 'partial' ? 'Partly right!' : result.verdict === 'pending' ? 'Host marking in progress' : result.verdict === 'unanswered' ? 'No answer submitted' : 'Not this time'
  return <div className={`player-reveal ${result?.verdict || 'pending'}`} role="status"><span className="player-reveal-icon">{result?.verdict === 'correct' ? '✓' : result?.verdict === 'partial' ? '◐' : result?.verdict === 'incorrect' || result?.verdict === 'unanswered' ? '×' : '✦'}</span><h2>{title}</h2><div className="player-reveal-detail"><small>YOUR ANSWER</small><strong>{answerLabel(response?.value)}</strong></div>{question.answer !== undefined && <div className="player-reveal-detail"><small>CORRECT ANSWER</small><strong>{answerLabel(question.answer)}</strong></div>}{result?.detail&&<p className="player-grade-detail">{result.detail}</p>}{question.explanation && <p>{question.explanation}</p>}{result && result.verdict !== 'pending' && <div className="player-reveal-points">{result.points.toLocaleString()} <span>points for this question</span></div>}</div>
}
  ...
  return <Shell active="Quiz editor"><main className="page editor-page">
    ...
    <div className="editor-grid">
      ...
      <div className="editor-form">
        ...
        {draft.type==='number'&&<div className="number-bands-editor"><strong>Number scoring bands</strong><span>First matching tolerance wins. Fractions run from 0.01 to 1.</span>{(draft.numberBands||[{tolerance:0,fraction:1}]).map((band,index)=><div className="form-two" key={`${band.tolerance}-${index}`}><label>Tolerance ±<input type="number" min="0" value={band.tolerance} onChange={event=>setDraft({...draft,numberBands:(draft.numberBands||[{tolerance:0,fraction:1}]).map((item,itemIndex)=>itemIndex===index?{...item,tolerance:Number(event.target.value)}:item)})}/></label><label>Score fraction<input type="number" min="0.01" max="1" step="0.05" value={band.fraction} onChange={event=>setDraft({...draft,numberBands:(draft.numberBands||[{tolerance:0,fraction:1}]).map((item,itemIndex)=>itemIndex===index?{...item,fraction:Number(event.target.value)}:item)})}/></label><button type="button" disabled={(draft.numberBands?.length||1)<=1} onClick={()=>setDraft({...draft,numberBands:(draft.numberBands||[]).filter((_,itemIndex)=>itemIndex!==index)})}><Trash2 size={15}/> Remove band</button></div>)}<button type="button" className="option-add" onClick={()=>setDraft({...draft,numberBands:[...(draft.numberBands||[{tolerance:0,fraction:1}]),{tolerance:5,fraction:.5}]})}><Plus size={15}/> Add scoring band</button></div>}
...
function Host() {
  ...
  const liveAction = async (type: 'advance' | 'break' | 'resume' | 'void' | 'grade' | 'grade-all-zero' | 'end', extras: { playerId?: string; points?: number } = {}) => {
    ...
    if (type === 'advance' && game.phase === 'reveal' && q?.type === 'free') {
      const pending = game.grades.filter(grade => grade.questionId === q.id && grade.verdict === 'pending').length
      if (pending && !confirm(`${pending} response${pending === 1 ? '' : 's'} still need marking. Continue and leave them unscored?`)) return
    }
    setLiveBusy(true)
    try {
  ...
  return <Shell active="Host console"><main className="page host-page">
    ...
    <div className="host-grid">
      <div className="host-main">
        ...
        <div className="response-card"><div className="card-title"><h3>Incoming answers</h3><Badge tone={game.phase==='open'?'green':'gray'}>{answered.length} / {(game.questionEligiblePlayerIds||game.players.map(player=>player.id)).length} submitted</Badge></div>{answered.length===0?<div className="empty-state">Player answers will appear here while the question is open.</div>:<div className="answer-list">{answered.map(response=>{const player=game.players.find(item=>item.id===response.playerId);if(!player)return null;const grade=gradeFor(game,player.id,q.id);return <div key={player.id} className="answer-row"><div className="answer-player"><PlayerAvatar player={player} packs={packs}/><span><strong>{player.name}</strong><small>{new Date(response.submittedAt).toLocaleTimeString()}</small></span></div><span className="response-value">{typeof response.value==='object'?answerLabel(response.value):String(response.value)}</span>{q.type==='free'&&game.phase==='reveal'&&<div className="mark-buttons"><button title="0%" disabled={!liveConnected||!canControl} onClick={()=>void liveAction('grade',{playerId:player.id,points:0})}>0%</button><button title="50%" disabled={!liveConnected||!canControl} onClick={()=>void liveAction('grade',{playerId:player.id,points:q.points*.5})}>50%</button><button title="100%" disabled={!liveConnected||!canControl} onClick={()=>void liveAction('grade',{playerId:player.id,points:q.points})}>100%</button></div>}{q.type!=='free'&&game.phase==='reveal'&&<button className="score-override" disabled={!liveConnected||!canControl} onClick={()=>{const raw=prompt(`Award points from 0 to ${q.points}`,String(grade?.points||0));if(raw!==null&&Number.isFinite(Number(raw)))void liveAction('grade',{playerId:player.id,points:Number(raw)})}}>Adjust</button>}{grade&&<Badge tone={grade.points?'green':'gray'}>{grade.points} pts · {grade.detail}</Badge>}</div>})}{q.type==='free'&&game.phase==='reveal'&&<button className="mark-remaining" disabled={!liveConnected||!canControl} onClick={()=>void liveAction('grade-all-zero')}>Mark all remaining 0</button>}</div>}</div>
      </div>
...
function Join() {
  ...
  const submit=async()=>{
    ...
    if (q.type === 'anagram') {
      if (Date.now() < anagramRetryAt.current) { setError('Wait a moment before guessing again.'); return }
      if (normalise(String(answer)) !== normalise(String(q.anagramSolution || q.answer || ''))) {
        anagramRetryAt.current = Date.now() + 1000
